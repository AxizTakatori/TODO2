...
public class User_infoDao {
	...
	public List<User_info2> findAll(){
		...
		try (PreparedStatement stmt = con.prepareStatement(SQL_SELECT_ALL)){
			...
			while (rs.next()) {
				User_info2 lg = new User_info2(rs.getInt("user_id"),rs.getString("user_name"),rs.getString("telephone"),rs.getString("password"));
				list.add(lg);
			}
		} catch (SQLException e) {
			throw new RuntimeException(e);
	...
	//Daoの登録メソッド
	public void register(String newname, String newtel, String newpass) {
